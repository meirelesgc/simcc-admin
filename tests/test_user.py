...
async def test_get_me(client, create_user, auth_header):
    user = await create_user()
    response = client.get('/user/me', headers=auth_header(user))
    assert response.status_code == HTTPStatus.OK
    data = user_model.UserResponse(**response.json())
    assert data.user_id == user.user_id
...
async def test_put_my_user(client, create_user, auth_header):
    ...
    assert response.status_code == HTTPStatus.OK
    updated = user_model.UserResponse(**response.json())
...
@pytest.mark.asyncio
async def test_user_cannot_promote_to_admin(client, create_user, auth_header):
    user = await create_user()

    update_data = user.model_dump(mode='json', exclude_unset=True)
